...
export default function Lista(){
    ...
    const listItems = autores.map(autor =>
        <tr className="bg-neutral-primary border-b border-default" key={autor.name}>
            ...
            <td>
                <Image src={autor.image} alt={autor.name} width={50} height={50} />
            </td>
            <td>
                <div className="d-flex flex-wrap gap-2 justify-content-end">
                    <button disabled={false} type="button" className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600" title="Editar">Editar</button>
                    <button disabled={false} type="button" className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600" title="Eliminar">Eliminar</button>
                </div>
            </td>
        </tr>
    );
